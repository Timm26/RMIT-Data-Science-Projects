...
# aus_eq <- read_csv(here("ZDatasets", "aus_eq"))
...
```{r}
starwars |> 
  dplyr::select(name, mass, height, BMI, weight_class) |>
  arrange(name) |> head()
```
...
starwars |> 
  dplyr::select(starts_with("B")) |> head()
```
...
starwars |> 
  ...
  arrange(height, mass) |> 
  group_by(hair_color) |> 
  mutate(unitid = glue("{name} {hair_color} {height} {mass}"), .keep = 'none') |> 
  head()
```
...
quakes <- quakes |>  dplyr::select(-stations) |> rename(latitude= lat, longitude = long)
auseq <- auseq |>  dplyr::select(latitude, longitude, depth, mag)
auseq |> head()
quakes |> tail()
```
...
starwars |> dplyr::select(sex, sex_short) |> head()
```

### 3.2.2 Using strsplit

```{r}
# Output will be a list
streets2 <- "Victoria-Yarra-Russell-Williams-Swanston"
strsplit(streets2, split = "-")
unlist(strsplit(streets2, split = "-")) #Unlist the list
```

## 3.3 Upper/ lower case conversion

```{r}
a <- "MATH2405 is AWsomE"
tolower(a)
toupper(a)
```

## 3.4 String abbreviations
...
## 3.5 Test equal strings

```{r}
# Do vectors contain the same values
set_3 <- c("VIC", "NSW", "TAS")
set_4 <- c("WA", "SA", "NSW")
set_5 <- c("NSW", "SA", "WA")
...
```{r}
# Use it in a filter - filter for id:"us70004m50"
aus_eq |> 
  dplyr::filter(id %in% "us70004m50")
```


## 3.6 Convert a vector to a string and split it

```{r}
x <- toString(c("Jul", 25, 2017)) # Convert the vector to a string
x
str_split(x, ", ") # Split the string on "comma space"
```

# 4. Manipulate strings using stringr

## 4.1 Basic string manipulation
...
*Workbook `R Basics` dives into the details.*
...
## 4.3 Extract and replace data

```{r}
# Change "Kobe Bryant" to "The Black Mamba"
kobe <- lakers %>% 
  mutate(player = str_replace_all(player, "Kobe Bryant", "The Black Mamba")) %>% 
  filter(str_detect(player, "Mamba"))
nrow(kobe) 

# Get all points
kobe_shots <- kobe %>% 
  filter(str_detect(result, "made")) %>% 
...
lubridate::now()
# In one dataframe
curr_details <- data.frame(Date = Sys.Date(),
                           Time = Sys.time(),
                           tz = Sys.timezone())
curr_details
```
...
## 6.2 Extract and manipulate date variables
...
# Extract the weekday
flights_new$departure |> wday(label = TRUE, abbr=FALSE) |> head()
```

## 6.3 Calculate time differences in a dataframe

```{r}
# Time difference in days
difftime(as.Date(aus_eq$time[42]), as.Date(aus_eq$time[96]))
# Time difference in weeks
difftime(as.Date(aus_eq$time[42]), as.Date(aus_eq$time[96]), units = "weeks")
```

# 7. Tokenisation & Stop Words (Coming Soon)
