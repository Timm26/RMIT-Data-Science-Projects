...
Crime_QLD <- read_csv(here("ZDatasets","QLD_Reported_Offences_Rates.csv"))

# Split the `Month Year` column into two separate columns
Crime_QLD %<>% tidyr::separate(`Month Year`, into = c("Month", "Year"), sep = 3)
...
Crime_NSW <- read_excel(here("ZDatasets",'NSW_Crime.xlsx'), sheet = "Summary of offences", range=cell_cols("Q:Z"))
...
Crime_WA <- read_excel(here("ZDatasets",'wa-police-force-crime-timeseries.xlsx'), sheet = 'Western Australia', skip = 7)
...
Crime_VIC <- read_excel(here("ZDatasets","Victoria_Crime.xlsx"), sheet="Table 01")
...
# Load the dataset
drug_cat_NSW <- read_excel(here("ZDatasets",'NSW_Crime.xlsx'), sheet = "Summary of offences", range=cell_cols("G:P"))
...
# Load the dataset
Drug_deaths <- read_excel(here("ZDatasets",'Drug_Death.xlsx'), sheet = "Table 1", skip =1)
...
Unemployment_NSW_VIC <- read_excel(here("ZDatasets","Unemployment_rate.xlsx"), sheet = "Data1")
Unemployment_NSW_VIC %<>% select(`...1`, `> New South Wales ;  Unemployment rate ;  Persons ;`,
                                 `> Victoria ;  Unemployment rate ;  Persons ;`)
# Load Unemployment rate NSW and VIC
Unemployment_QLD <- read_excel(here("ZDatasets","Unemployment_rate.xlsx"), sheet = "Data2")
Unemployment_QLD %<>% select(`...1`, `> Queensland ;  Unemployment rate ;  Persons ;`)
# Load Unemployment rate NSW and VIC
Unemployment_WA <- read_excel(here("ZDatasets","Unemployment_rate.xlsx"), sheet = "Data3")
