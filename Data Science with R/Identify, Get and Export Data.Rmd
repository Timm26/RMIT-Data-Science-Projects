...
# Standard R function
iris1 <- read.csv(here('ZDatasets','iris.csv'), stringsAsFactors = TRUE) # Will read factors as <fctr>
head(iris1, n = 5)
```
...
iris2 <- read.table(here('ZDatasets','iris.csv'), sep = ',', header = TRUE, row.names = 1) # as indexcol is given
head(iris2, n=3)
```
...
# Load the package
library(readr)
iris3 <- read_csv(here("ZDatasets","iris.csv"))
iris3 <- column_to_rownames(iris3, var = colnames(iris3)[1]) # changes the first column to the index
head(iris3, n=3)
```

## 1,4 Excel Spreadsheets
## 1.4.1 {readxl} package

```{r}
library(readxl) # read_excel() returns a tibble
# Skip 1 row and add colnames
iris4 <- read_excel(here("ZDatasets","iris.xlsx"), sheet = 'iris', skip = 1, col_names = paste("Var", 1:5))
head(iris4, 3)
```

## 1.4.2 {openxlsx} package

```{r}
grades <- openxlsx::read.xlsx(here("ZDatasets","Grade dataset.xlsx")) # just 1 sheet
head(grades)
```
...
# Export as .csv
df |> write_csv(here("ZDatasets","export_example1.csv")) # path = "" -> to change the path
df |> write_csv(here("ZDatasets","export_example2.csv"), col_names = FALSE) # excl. colnames
df |> write.csv(here("ZDatasets","export_example3.csv"), row.names = FALSE) # excl. rownames
df |> write_delim(here("ZDatasets","export_example1.txt")) # Export df as a text file
```
