...
In this instance we do not have any left-skewed data, if we did, we could use a power transformation or boxcox transformation. Let's apply a boxcox transformation on `revenue`:

```{r}
# Boxcox requires strictly positive values
all(copy$revenue > 0)

# Since this is true - apply Boxcox
bc <- forecast::BoxCox(copy$revenue, lambda = "auto")
lambda <- round(attr(bc, which = "lambda"),3)

# Display both plots
par(mfrow = c(2,1))
hist(copy$revenue, main = "Histogram of Revenue before BoxCox transformation", breaks = 25,
     xlab = "Revenue (original)")
hist(bc, main = glue::glue("Histogram of Revenue after BoxCox transformation {lambda}"),
     breaks = 25, col='green', xlab = "Revenue (transformed)")
```

This transformation didn't really add any value and our data is still not normally distributed:

```{r}
# Shapiro Wilk test - p <= 0.05 reject H_0 of normality
shapiro.test(bc)
```

This power transformation didnt change the distribution.

## Summary statistics

The below calculates several summary statistics of our GYG dataset.
...
copy |> 
  summarise(
    mean_rev_per_head = mean(revenue_per_head, na.rm=TRUE), # all na values are removed
    min_staff = min(staff, na.rm = TRUE),
    total_stores = n_distinct(store_id, na.rm = TRUE),
    median_happy_score = median(round(happy_score,3), na.rm = TRUE),
    max_age = max(age),
    .groups = "drop") # ensures the resulting df will no longer be grouping var after the
                      # result has been displayed (Tidyverse, 2016)
```

This finishes the analysis of our synthetic dataset.
